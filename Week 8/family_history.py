...
def print_death_age(people_dict):
    ...
    for values in people_dict.values():
        ...
        death_age = values[DEATH_YEAR_INDEX] - values[BIRTH_YEAR_INDEX]
...
def count_genders(people_dict):
    ...
    female_count = 0
    for values in people_dict.values():
        if values[GENDER_INDEX] == "M":
            male_count += 1
        elif values[GENDER_INDEX] == "F":
            female_count += 1
...
def print_marriages(marriages_dict, people_dict):
    ...
    print("Marriages:")
    # marriage_key: [husband_key, wife_key, wedding_year]
    for value in marriages_dict.values():
        # print(value)
        husband_id = value[HUSBAND_KEY_INDEX]
        wife_id = value[WIFE_KEY_INDEX]
        marriage_year = value[WEDDING_YEAR_INDEX]

        # print(f'{husband_id}, {wife_id}, {marriage_year}')
        
        # person_key: [name, gender, birth_year, death_year]
        # HUSBAND INFO
        husband = people_dict.get(husband_id)
        husband_name = husband[NAME_INDEX]
        husband_age = marriage_year - husband[BIRTH_YEAR_INDEX]
        
        # WIFE INFO
        wife = people_dict.get(wife_id)
        wife_name = wife[NAME_INDEX]
        wife_age = marriage_year - wife[BIRTH_YEAR_INDEX]

        print(f"{husband_name} {husband_age} > {marriage_year} < {wife_name} {wife_age}")
    

# def total_marriages(marriages_dict, people_dict):
#     """Calculate how many marriages each person had.
#     """
#     total_marriages = {} # keeps track of each person's marriage.
#     for value in marriages_dict.values():
#         # print(value)
#         husband_id = value[HUSBAND_KEY_INDEX]
#         wife_id = value[WIFE_KEY_INDEX]
